...
| Layer | Intent |
|-------|--------|
| **Goal** | Search stack doc prevents wrong architecture (e.g. client-side only search). |
| **Workflow** | Readers follow anti-patterns table; update when edges change. |
| **Proof** | Diagram matches current `ai-search` + embeddings path. |
| **Gates** | Reference doc — verify quarterly. |
| **Rollout** | N/A (documentation). |
