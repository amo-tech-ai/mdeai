...
| Layer | Intent |
|-------|--------|
| **Goal** | OpenClaw gateway adapter routes tasks with health and idempotency. |
| **Workflow** | Map events → adapter → Hermes/Paperclip; handle retries. |
| **Proof** | Duplicate delivery does not duplicate side effects. |
| **Gates** | 05M health before prod traffic. |
| **Rollout** | Canary channel volume. |
