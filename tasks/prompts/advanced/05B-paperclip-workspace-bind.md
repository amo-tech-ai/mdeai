...
| Layer | Intent |
|-------|--------|
| **Goal** | Workspace bind path is repeatable for engineers cloning the repo. |
| **Workflow** | Script or doc path → verify bind → commit template env. |
| **Proof** | Fresh clone follows README steps without `/home/sk` hardcoding. |
| **Gates** | No secrets in repo; use env vars. |
| **Rollout** | Internal tooling only. |
