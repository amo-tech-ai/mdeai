...
| Layer | Intent |
|-------|--------|
| **Goal** | Approval gates G1–G7 block risky actions until criteria met. |
| **Workflow** | Define gate checks → integrate notifications → test deny/allow. |
| **Proof** | Denied action returns structured reason; audit entry if 05K resolved. |
| **Gates** | Depends on 05K for persistent audit if required. |
| **Rollout** | Internal ops tooling first. |
