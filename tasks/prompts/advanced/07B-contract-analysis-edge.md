...
| Layer | Intent |
|-------|--------|
| **Goal** | Lease PDF/text analysis returns risks via Gemini with citations. |
| **Workflow** | Edge upload path → Gemini → structured response → log `ai_runs`. |
| **Proof** | Known fixture leases produce expected risk flags. |
| **Gates** | G1–G5; no raw key in logs. |
| **Rollout** | User consent for document upload. |
