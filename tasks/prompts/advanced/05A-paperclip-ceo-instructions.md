...
| Layer | Intent |
|-------|--------|
| **Goal** | Paperclip CEO has actionable instructions for delegation and budgets — ops can run playbooks. |
| **Workflow** | Edit prompt/config → validate with Paperclip API smoke → document rollback. |
| **Proof** | Heartbeat or test issue created; no plaintext secrets in instructions. |
| **Gates** | Paperclip credentials in env only; Hermes CLI path documented. |
| **Rollout** | Staging org first; prod after review. |
