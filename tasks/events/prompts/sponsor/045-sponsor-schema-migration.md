---
task_id: 045-sponsor-schema-migration
title: sponsor.* schema migration — 10 tables + RLS + FK indexes
...
edge_function: null
schema_tables:
  - sponsor.organizations       # NEW
  - sponsor.applications        # NEW
  - sponsor.assets              # NEW
  - sponsor.placements          # NEW
  - sponsor.impressions         # NEW
  - sponsor.clicks              # NEW
  - sponsor.attributions        # NEW
  - sponsor.roi_daily           # NEW
  - sponsor.invoices            # NEW
...
| Aspect | Details |
|---|---|
| **Phase** | PHASE-1-SPONSOR-MVP — gate 0: nothing else builds without this |
| **Schema** | New `sponsor` schema; 9 tables; RLS on all |
| **Real-world** | Postobón applies for Gold tier of "Reina de Antioquia 2026." `sponsor.organizations` holds their NIT + legal name; `sponsor.applications` captures the tier + activation type; `sponsor.placements` becomes the single source of truth for every brand surface |

## Description

**The situation.** No sponsorship tables exist. All 13 subsequent tasks (045–058) depend on this migration. Until these tables land, there is no safe place to store sponsor data.

**Why a separate `sponsor` schema not `public.*`.** The sponsorship surface is owned by a different product persona (brands/agencies) than the event organizer flow. Namespace isolation prevents FK sprawl in `public.*`, makes RLS simpler to reason about, and lets a future tenant-isolation upgrade touch one schema.

**What already exists.** `public.events`, `public.profiles`, `auth.users` are all live. The `sponsor.*` tables FK into these. The migration runner (`supabase db push`) and advisor workflow are established from Phase 1.5.

**The build.** One migration file creating: `sponsor` schema, 9 tables, indexes, RLS policies, one `updated_at` trigger, and grants to `authenticated` and `service_role`.
...
-- 2. One application = one brand + one contest + one activation type + tier
CREATE TABLE sponsor.applications (
  ...
  activation_type  text NOT NULL CHECK (activation_type IN
    ('title_naming','category_powered_by','contestant_sponsor','venue_sponsor','digital')),
  tier             text NOT NULL CHECK (tier IN ('bronze','silver','gold','premium','custom')),
  pricing_model    text NOT NULL CHECK (pricing_model IN ('flat','cpl','cpa','cpm','hybrid')),
...
CREATE TABLE sponsor.assets (
  ...
  kind                  text NOT NULL CHECK (kind IN ('logo','video','image','copy','color')),
  storage_path          text,
  alt_text              text,
...
-- 4. Placements — where a brand actually shows up (one application → N placements)
CREATE TABLE sponsor.placements (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  application_id  uuid NOT NULL REFERENCES sponsor.applications(id) ON DELETE CASCADE,
  surface         text NOT NULL,          -- 'contest_header','leaderboard_footer','contestant_profile','wa_broadcast','qr_station','push_notif','social_post','category_header','digital_banner'
  surface_ref     uuid,                   -- event_id / entity_id / category_id depends on surface
...
-- 7. Attribution: vote/conversion linked to a placement (24h last-click model)
CREATE TABLE sponsor.attributions (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  placement_id     uuid NOT NULL REFERENCES sponsor.placements(id) ON DELETE CASCADE,
  click_id         uuid REFERENCES sponsor.clicks(id),
  conversion_kind  text NOT NULL CHECK (conversion_kind IN ('vote','signup','purchase','redemption')),
  conversion_value_cents int NOT NULL DEFAULT 0,
  attributed_at    timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX sponsor_attributions_placement_ts_idx ON sponsor.attributions(placement_id, attributed_at);
...
```sql
-- organizations: owner + service role
...
-- applications: sponsor sees own org's; service role writes
-- ⚠️ NOTE: No UPDATE policy for authenticated role — all admin status changes
--   (approve, reject) must use service_role client in edge functions (bypasses RLS).
--   The approve_sponsor_application RPC uses SECURITY DEFINER to run as service_role.
...
-- Sponsor can UPDATE only their own draft applications (status='draft' edits in wizard)
CREATE POLICY sponsor_apps_org_update ON sponsor.applications FOR UPDATE
  USING (organization_id IN (
    SELECT id FROM sponsor.organizations
    WHERE primary_contact_user_id = (select auth.uid())
  ) AND status = 'draft')
...
-- placements: public SELECT for active; sponsor SELECT for own
...
-- impressions, clicks, attributions: edge-fn writes only (service_role)
-- roi_daily: sponsor reads own; cron writes
...
- [ ] `sponsor` schema exists; all 9 tables created with correct columns and constraints.
- [ ] All FK indexes present; `get_advisors(type: "performance")` returns no new FK-index errors on `sponsor.*`.
- [ ] `get_advisors(type: "security")` shows RLS enabled on all 9 tables.
- [ ] Sponsor org owner can INSERT/SELECT own organization and applications.
- [ ] Sponsor cannot SELECT another org's applications (RLS isolation).
- [ ] Active public placement is readable by unauthenticated users via SELECT (needed for `<SponsoredSurface>`).
- [ ] `service_role` can write to impressions, clicks, attributions (edge fns use service client).
- [ ] `npm run verify:edge` passes after migration.

## See also

- [`tasks/events/03-sponsorship-system.md`](03-sponsorship-system.md) §3 — full schema rationale
- [`046-sponsor-apply-wizard.md`](046-sponsor-apply-wizard.md) — depends on this
- [`055-sponsor-contracts-schema.md`](055-sponsor-contracts-schema.md) — adds `sponsor.contracts` table
