...
> **Last updated:** 2026-04-30 — **D5 + D6 + D7 + D7.5 shipped same session.** D5: listing-create + wizard 1→4. D6: listing-moderate HMAC magic-link. D7: /host/dashboard + RoleProtectedRoute + ListingCard. D7.5 (NEW today): WhatsApp tap-to-chat lead capture — `lead-from-form` edge fn (verify_jwt:false, honeypot, IP+session rate limit) + `WhatsAppContactModal` (3 fields, then wa.me opens, then "did you send it?" confirm) + `whatsapp-deeplink` util. Deferred SMS OTP to V2 — `wa.me` deep-link is the verification signal. 113 vitest (was 86, +27 across D7+D7.5); 37 deno tests; 10/10 chunks within budget; entry 95.63 KB gzip. **Next:** D8 — auto_create_lead_from_message DB trigger + `lead-classify` edge fn (Gemini Flash for structured profile extraction). Form-channel leads come pre-structured so lead-classify is cheap (only runs on raw_message field for chat leads). Sweep-ups: R-RLS-1 (apartments policy lockdown), and a deno test for lead-from-form lands with D9 inbox work.
