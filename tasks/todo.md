...
> **Last updated:** 2026-04-30 — **D5 + D6 shipped end-to-end same day.** D5: `listing-create` edge fn live, full wizard 1→4 verified, both verdicts (auto_approved/needs_review/rejected) browser-proven, 5 photo upload to listing-photos bucket. D6: `listing-moderate` magic-link edge fn live, HMAC token signing/verifying, `_shared/founder-email.ts` stub (logs to stdout + optional webhook), email-fire hook on `needs_review` in listing-create. 37 deno tests passing (was 25 before D6). Found + fixed 2 bugs during browser proof: `apartments.status` CHECK constraint disallows `'hidden'` (changed to `'inactive'`); HTML title typo `'rejectd'` (added explicit `actionPastTense()` helper). All five gates green. **Next:** D7 host dashboard shell + listings list + RoleProtectedRoute per `tasks/plan/06-landlord-v1-30day.md` §5.1.
