...
## 🎟️ EVENTS + CONTESTS BACKLOG (2026-05-03 — full Phase 1-4 roster)

> Master roster + per-task testing strategy + success criteria. Full specs live at [`tasks/events/prompts/<NNN>-*.md`](./events/prompts/000-index.md). Build order respects YAML `depends_on`; layer batching is **suggested**, not strict.

### Skills to invoke per task type (always run skills first)

| Task type | Skills (in order) |
|---|---|
| Schema migration / RLS / RPC | `supabase` → `supabase-postgres-best-practices` → `mdeai-project-gates` |
| Edge function (Deno) | `supabase-edge-functions` → `supabase` → `mdeai-project-gates` |
| Gemini AI integration | `gemini` → `supabase-edge-functions` → `mdeai-project-gates` |
| React page / form / wizard | `frontend-design` → `vitest-component-testing` → `claude-preview-browser-testing` → `mdeai-project-gates` |
| Real-estate domain logic | `real-estate-mdeai` (V1) or `real-estate-tech` (V2) |
| Mermaid diagram | `mermaid-diagrams` |
| Plan / runbook | `mde-writing-plans` |
| Commit / PR | `git-commit` → `commit-commands:commit-push-pr` |

### Phase 1 EVENTS (10 tasks — P0 launch blockers + 1 P1) — STATUS 2026-05-03

| # | Task | Effort | Status | Skills | Testing strategy | Success criteria |
|---|---|---|---|---|---|---|
| 001 | [event-schema-migration](./events/prompts/001-event-schema-migration.md) | 1.5d | ✅ DONE 05-03 | supabase, supabase-postgres-best-practices, mdeai-project-gates | `apply_migration` to live + `get_advisors security` (zero new errors) + `execute_sql` smoke test on each RPC | Schema deployed; 6 tables visible; 10 RPCs invocable; advisor count steady or down |
| 002 | [host-event-new-wizard](./events/prompts/002-host-event-new-wizard.md) | 3d | ✅ DONE 05-03 | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest unit per step + Claude Preview MCP `/host/event/new` E2E (Sofía persona): publishes "Reina de Antioquia" with 4 tiers in <10 min | Lighthouse a11y ≥90; build green; 1 published event in DB; 4 tickets seeded |
| 034 | [event-staff-link-generator](./events/prompts/034-event-staff-link-generator-edge-fn.md) | 0.5d | ✅ DONE 05-03 | supabase-edge-functions, mdeai-project-gates | curl matrix on deployed fn (3/3 pass) + `bump_staff_link_version` SQL smoke test (1→2 verified) | Returns valid 24h JWT URL; revoke endpoint bumps version; advisor count down 2 from RPC lockdown |
| 004 | [ticket-checkout](./events/prompts/004-ticket-checkout-edge-fn.md) | 2d | ✅ DONE 05-03 | supabase-edge-functions, supabase, mdeai-project-gates | curl boundary matrix (8/8 pass) + RPC happy-path SQL test + OUT_OF_STOCK guard + cancel flow + finalize flow (12/12 total) | Atomic capacity guard verified (qty_pending invariant holds); ATTENDEES_MISMATCH + quantity bounds enforced; idempotency cache works |
| 005 | [ticket-payment-webhook](./events/prompts/005-ticket-payment-webhook.md) | 1d | ✅ DONE 05-03 (RPC chain proven via 004 test #12) | supabase-edge-functions, supabase, mdeai-project-gates | curl boundary (METHOD/CONFIG checks pass) + Stripe sig path requires real keys (deferred until secrets set) | Bad signature → 400; replay → 200 no-op; finalize RPC flips order pending→paid + attendees pending→active + qty transferred |
| 006 | [ticket-validate](./events/prompts/006-ticket-validate-edge-fn.md) | 1d | ✅ DONE 05-03 | supabase-edge-functions, supabase, mdeai-project-gates | curl boundary (4/4 pass) + RPC consume happy path + double-scan → already_used + unknown_token + audit log insert | First scan returns `consumed`; second scan returns `already_used` with original timestamp; `event_check_ins` row written for every scan attempt |
| 003 | [host-event-dashboard](./events/prompts/003-host-event-dashboard.md) | 2d | ⏳ TODO | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest per-component + Claude Preview E2E (Sofía persona): KPIs render, Realtime updates, "Generate staff link" button calls 034 fn, table renders attendees from seed | KPIs accurate to the cent vs Stripe Dashboard; Realtime tile updates within 2s of new check-in; revoke link → next scan returns STAFF_TOKEN_REVOKED |
| 008 | [me-tickets-page](./events/prompts/008-me-tickets-page.md) | 1d | ⏳ TODO | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest unit + Claude Preview E2E (Camila persona): `/me/tickets/<order_id>?token=<access>` shows QR + .ics download | QR PNG renders ≥256px; access_token gate works (wrong token → 404); .ics opens in Google Calendar |
| 007 | [staff-checkin-pwa](./events/prompts/007-staff-checkin-pwa.md) | 1.5d | ⏳ TODO | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest scanner state machine + Claude Preview E2E (Andrés persona, Pixel 7 viewport, offline mode): scan QR → green ✓ in 250ms; rescan → red ✗ ALREADY_USED | <500ms p95 4G; offline queue replays on reconnect; 100 concurrent scan stress test → 1 success / 99 already_used |
| 009 | [chatbot-event-creation](./events/prompts/009-chatbot-event-creation.md) | 2d | ⏳ TODO (P1) | gemini, supabase-edge-functions, mdeai-project-gates | Vitest + Claude Preview MCP E2E: open chatbot, ask "create music festival in Poblado for 200 people", agent calls `create_event_draft` + `search_venues` (tool combination per task 045) | Tool combination + thoughtSignature preservation working; chatbot can publish a draft event end-to-end without leaving the chat surface |

**Phase 1 acceptance gate (cannot ship Phase 2 without all 7 green):**
- ✅ Schema deployed (001) + advisor clean
- ✅ Wizard publishes event end-to-end (002)
- ⏳ Camila buys via Stripe → email arrives → QR shown at `/me/tickets` (depends 008 + secrets)
- ⏳ Andrés scans QR → green; rescan → red ✗ ALREADY_USED; logged to `event_check_ins` (depends 007)
- ⏳ Sofía's dashboard reconciles to the cent vs Stripe (depends 003)
- ⏳ Load test: 50 concurrent buyers on 10-ticket tier → exactly 10 sold, 40 OUT_OF_STOCK, 0 oversell (RWT)
- ⏳ Revoking staff_link_version invalidates outstanding scanner JWTs (verified via 034)
- ⏳ Lighthouse a11y ≥90 on 4 new screens

### Phase 1.5 EVENTS (7 tasks — fast-follow hardening, ships after launch)

| # | Task | Effort | Status | Skills | Testing strategy | Success criteria |
|---|---|---|---|---|---|---|
| 025 | [event-promo-codes-schema](./events/prompts/025-event-promo-codes-schema.md) | 0.5d | ⏳ TODO | supabase, supabase-postgres-best-practices, mdeai-project-gates | `apply_migration` + RLS smoke test + advisor pass | 100% sponsor comp tickets work; HE-2 critical gap closed |
| 026 | [event-order-refunds-schema](./events/prompts/026-event-order-refunds-schema.md) | 0.5d | ⏳ TODO | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + partial-refund + chargeback unit tests via SQL | Partial refunds reconcile to Stripe; audit trail per refund |
| 027 | [event-taxes-and-fees-schema](./events/prompts/027-event-taxes-and-fees-schema.md) | 1d | ⏳ TODO | supabase, mdeai-project-gates | Tax-calc unit tests + integration with checkout RPC | Colombia IVA 19% applied; legal organizer invoicing works |
| 030 | [event-media-assets-schema](./events/prompts/030-event-media-assets-schema.md) | 0.5d | ⏳ TODO | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + storage policy test + RLS pass | Sponsor logos persisted; 033 verdicts stored alongside media |
| 033 | [event-photo-moderate-edge-fn](./events/prompts/033-event-photo-moderate-edge-fn.md) | 0.5d | ⏳ TODO | gemini, supabase-edge-functions, mdeai-project-gates | curl with 4 asset_type variants (logo/gallery/flyer/profile) + verdict comparison | mediaResolution per asset_type saves 4× tokens on logos; verdict matches expected enum |
| 045 | [gemini-native-sdk-migration](./events/prompts/045-gemini-native-sdk-migration.md) | 1d | ✅ DONE 05-03 | gemini, supabase-edge-functions, mdeai-project-gates | Lint/build/tests pass; deprecation grep returns 0 | All 5 simpler callers migrated; helper enforces G1 (responseJsonSchema required) at type level; no temperature usage |
| 046 | [gemini-skill-housekeeping](./events/prompts/046-gemini-skill-housekeeping.md) | 0.5d | ✅ DONE 05-03 | mde-writing-plans | Self-audit: 3 new reference files written; SKILL.md Resources + Checklist updated | Future Claude reads accurate tool-combination guidance + Maps + Code Execution refs |

### Phase 2 CONTESTS (15 tasks — voting layer on top of Phase 1 events) — see [`tasks/events/prompts/010..024`](./events/prompts/000-index.md)

> Cannot start Phase 2 without all 7 Phase 1 gate items green. Layer batching:
...
> - Layer 3: 011 vote-cast (010+015+016), 018 contestant-intake (010+020), 019 admin-moderation (010+020), 022 broadcast (010+014+021)
> - Layer 4: 012 vote-page (011), 013 realtime-leaderboard (012), 017 fraud-scan-cron (011+014), 023 pg-cron-backstop (022), 024 trust-page (014)

| # | Task | Effort | Skills | Testing strategy | Success criteria |
|---|---|---|---|---|---|
| 010 | [vote-schema](./events/prompts/010-vote-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + 3 RPC unit tests via SQL | Vote insert atomic; UNIQUE on (contest_id, voter_phone_e164) honoured |
| 011 | [vote-cast-edge-fn](./events/prompts/011-vote-cast-edge-fn.md) | 1d | supabase-edge-functions, mdeai-project-gates | curl boundary + Turnstile mock + double-vote rejection + rate-limit per phone | 1 vote per phone per contest; Turnstile required; Sentry captures bad-token attempts |
| 012 | [vote-page-mobile](./events/prompts/012-vote-page-mobile.md) | 1.5d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest contestant card + Claude Preview MCP (Camila persona, mobile viewport, slow 3G): vote flow under 30s | LCP <2.5s on 4G; 1 vote/min/IP rate gate visible to user |
| 013 | [realtime-leaderboard](./events/prompts/013-realtime-leaderboard.md) | 1d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest + Claude Preview MCP (2 tabs): vote in tab 1 → tab 2 leaderboard updates within 2s | Realtime channel updates within 2s; ranking ties resolved by hybrid_score then vote_count then earliest |
| 014 | [hybrid-scoring-trigger](./events/prompts/014-hybrid-scoring-trigger.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | SQL trigger unit test: 100 votes + 5 judges → expected hybrid_score | Hybrid weight (votes 50% + judges 50%) tunable per contest; trigger fires on every vote insert |
| 015 | [cloudflare-turnstile](./events/prompts/015-cloudflare-turnstile.md) | 0.5d | mdeai-project-gates | Vitest mock + manual sitekey verification | Bot-traffic 99%+ blocked; UX adds <300ms |
| 016 | [phone-otp](./events/prompts/016-phone-otp.md) | 1d | supabase-edge-functions, supabase, mdeai-project-gates | curl boundary + Twilio mock + Colombia E.164 acceptance | OTP arrives in <10s; replay attempts blocked; rate limit per phone |
| 017 | [fraud-scan-cron](./events/prompts/017-fraud-scan-cron.md) | 1d | supabase-edge-functions, mdeai-project-gates | pg_cron schedule test + synthetic burst test (1000 votes/30s from same /24) | Bursts flagged within 5 min; false-positive rate <2% on legit traffic |
| 018 | [contestant-intake-form](./events/prompts/018-contestant-intake-form.md) | 1d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest form steps + Claude Preview MCP (Daniela persona): submit application end-to-end | Photo upload + RLS + 020 moderation verdict captured; admin sees pending row |
| 019 | [admin-moderation-page](./events/prompts/019-admin-moderation-page.md) | 1.5d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest + Claude Preview MCP (admin persona): approve / reject 5 contestants in <30s | Approve/reject flips contestant.status; audit log row per decision |
| 020 | [gemini-photo-moderation](./events/prompts/020-gemini-photo-moderation.md) | 1d | gemini, supabase-edge-functions, mdeai-project-gates | curl with safe + unsafe images + edge case (low resolution) | Verdict matches expected enum; uses callGeminiStructured (G1) per task 045 |
| 021 | [openclaw-vps-provision](./events/prompts/021-openclaw-vps-provision.md) | 1d | open-claw, mdeai-project-gates | SSH smoke test + heartbeat ping after install | OpenClaw daemon up + reachable from edge fns |
| 022 | [leaderboard-broadcast-skill](./events/prompts/022-leaderboard-broadcast-skill.md) | 1.5d | open-claw, mdeai-project-gates | Skill smoke test + 1 broadcast post per channel | WhatsApp/IG/Twitter posts dispatched on threshold change; rate limit honoured |
| 023 | [pg-cron-backstop](./events/prompts/023-pg-cron-backstop.md) | 0.5d | supabase, mdeai-project-gates | pg_cron registration + 1 firing per task | Backstop fires if OpenClaw offline; idempotent re-firing |
| 024 | [trust-page](./events/prompts/024-trust-page.md) | 1d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest + Claude Preview MCP (anon persona): trust page displays current rules + judge breakdown | Trust page live at `/contests/:slug/trust`; cites hybrid weights + 015/017 fraud measures |

### Phase 2 EVENTS (4 tasks — production-ops layer, ships alongside contests)

| # | Task | Effort | Skills | Testing strategy | Success criteria |
|---|---|---|---|---|---|
| 028 | [event-stakeholders-schema](./events/prompts/028-event-stakeholders-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + RLS test (planner can read but not edit organizer's notes) | Judges + planners visible in dashboard; no privilege escalation |
| 029 | [event-vendors-schema](./events/prompts/029-event-vendors-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + vendor-link smoke | Photographer/AV/security/catering trackable per event |
| 031 | [event-sponsors-link-schema](./events/prompts/031-event-sponsors-link-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + Phase 3 sponsor.* bridge test | Bridge table ready for Phase 3 sponsor system |
| 032 | [event-attendee-profiles-schema](./events/prompts/032-event-attendee-profiles-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + custom_fields JSONB schema validation | Dietary/accessibility/company fields persisted per attendee |

### Venue Management (10 tasks — Phase 1–4 across all phases) — see [`tasks/events/prompts/035..044`](./events/prompts/000-index.md)

| Phase | # | Task | Effort | Skills | Testing strategy | Success criteria |
|---|---|---|---|---|---|---|
| 1 | 035 | [venue-picker-in-wizard](./events/prompts/035-venue-picker-in-wizard.md) | 0.5d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest autocomplete + Claude Preview MCP: select existing or create-new inline | Autocomplete returns ≥3 venues within 200ms; new venue creates `event_venues` row |
| 2 | 036 | [event-venue-resources-schema](./events/prompts/036-event-venue-resources-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + AV/catering/furniture inventory smoke | Per-venue inventory readable + RLS scoped to venue owner |
| 2 | 037 | [event-venue-staff-schema](./events/prompts/037-event-venue-staff-schema.md) | 0.5d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + staff role smoke | Security/AV/catering ops staff trackable per venue |
| 2 | 038 | [event-venue-availability-schema](./events/prompts/038-event-venue-availability-schema.md) | 1d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + iCal RRULE parser test | Open/blocked windows scheduleable; recurrence honored |
| 2 | 039 | [host-venue-management-page](./events/prompts/039-host-venue-management-page.md) | 1.5d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest per-tab + Claude Preview MCP (Sofía persona): manage all 4 tabs | All 4 tabs render KPIs; Realtime updates; bookings tab paginates |
| 3 | 040 | [event-venue-layouts-schema](./events/prompts/040-event-venue-layouts-schema.md) | 1d | supabase, supabase-postgres-best-practices, mdeai-project-gates | Migration + layout zone test | Floor plans + zones (theater/banquet/reception) persisted |
| 3 | 041 | [event-venue-bookings-schema](./events/prompts/041-event-venue-bookings-schema.md) | 1d | supabase, supabase-postgres-best-practices, mdeai-project-gates | EXCLUDE USING gist double-booking test (concurrent insert → 1 wins, 1 fails) | Race-safe at DB level; contracts attachable |
| 3 | 042 | [venue-analytics-dashboard](./events/prompts/042-venue-analytics-dashboard.md) | 1d | frontend-design, vitest-component-testing, claude-preview-browser-testing, mdeai-project-gates | Vitest KPI components + Claude Preview MCP: data displays correctly with seed | Utilization %, revenue $, resource cost $, staff hours all visible |
| 4 | 043 | [ai-venue-optimizer-edge-fn](./events/prompts/043-ai-venue-optimizer-edge-fn.md) | 1.5d | gemini, supabase-edge-functions, mdeai-project-gates | curl test with sample venue → expected pricing/scheduling JSON; tool combination (Maps+Search+Code Exec+custom fns) per task 045 | callGeminiAgent works; thinkingLevel: high; recommendations actionable |
| 4 | 044 | [ai-venue-layout-generator-edge-fn](./events/prompts/044-ai-venue-layout-generator-edge-fn.md) | 1d | gemini, supabase-edge-functions, mdeai-project-gates | curl with 25m×40m test venue + sight-line constraints; verify Code Execution path | Python sandbox reasoning produces valid floor plan JSON; aisle widths ≥1.5m enforced |

---

## ⏳ TODO — PHASE-3-SPONSORS (14 tasks, ~10 dev-days)

Full sponsorship system. Human-approval gate at every payment step. Build order: 045 → 046 → 047 → 048 → 049/050 → 051 → 052 → 053 → 054 → 055 → 056 → 057 → 058.

> **Numbering note (2026-05-03):** disk files 045–058 ARE sponsor tasks. The gemini tasks previously listed at 045/046 in the index were completed inline in this session without separate files.

| Order | # | Task | Effort | Skills | Verify how | Definition of done |
|---|---|---|---|---|---|---|
| 1 | 045 | [sponsor-schema-migration](045-sponsor-schema-migration.md) | 1d | supabase, supabase-postgres-best-practices | `\dt sponsor.*`; get_advisors(security+performance) clean | ✅ DONE 05-04 — 9 tables created; RLS on all; UNIQUE constraint added (H1 fix) |
| 2 | 046 | [sponsor-apply-wizard](046-sponsor-apply-wizard.md) | 1d | frontend-design, supabase, mdeai-project-gates | Browser: Daniela completes apply wizard end-to-end | ✅ DONE 05-04 — 4-step wizard committed; ⚠️ H3 rejected_by cols pending |
| 3 | 047 | [sponsor-admin-queue](047-sponsor-admin-queue.md) | 1d | frontend-design, supabase, mdeai-project-gates | Browser: admin approves/rejects application | ✅ DONE 05-04 — queue renders; ⚠️ H2 placement surfaces hardcoded |
| 4 | 048 | [sponsor-stripe-checkout](048-sponsor-stripe-checkout.md) | 1d | supabase, mdeai-project-gates | curl matrix; Stripe test card purchase | ✅ DONE 05-04 — edge fn committed; ⚠️ needs STRIPE_SECRET_KEY secret + sponsor-assets bucket |
| 5 | 049 | [sponsor-surface-component](049-sponsor-surface-component.md) | 0.5d | frontend-design, mdeai-project-gates | Browser: logo renders on event page | ✅ DONE 05-04 — `<SponsoredSurface>` committed; impression edge fn wired |
| 6 | 050 | [sponsor-impression-click-edge-fns](050-sponsor-impression-click-edge-fns.md) | 0.5d | supabase, mdeai-project-gates | curl matrix | ✅ DONE 05-04 — both edge fns committed; ⚠️ M2 viewer_user_id missing in impressions |
| 7 | 051 | [sponsor-attribution-trigger](051-sponsor-attribution-trigger.md) | 0.5d | supabase, mdeai-project-gates | SQL trigger test | ✅ DONE 05-04 — trigger deployed to prod (C1 viewer_user_id fix applied) |
| 8 | 052 | [sponsor-dashboard](052-sponsor-dashboard.md) | 1d | frontend-design, supabase, mdeai-project-gates | Browser: Daniela views ROI tiles | ✅ DONE 05-04 — CPS/CPL/CPA tiles committed; ⚠️ M1 staleTime missing |
| 9 | 053 | [sponsor-roi-rollup-cron](053-sponsor-roi-rollup-cron.md) | 0.5d | supabase, mdeai-project-gates | pg_cron fires; `roi_daily` rows upserted | ✅ DONE 05-04 — cron deployed to prod (C2 idempotent unschedule fix applied) |
| 10 | 054 | [sponsor-ai-edge-fns](054-sponsor-ai-edge-fns.md) | 1d | gemini, supabase, mdeai-project-gates | curl → narrative sentence in Spanish | ⏳ TODO |
| 11 | 055 | [sponsor-contracts-schema](055-sponsor-contracts-schema.md) | 0.5d | supabase, mdeai-project-gates | `\dt sponsor.contracts`; advisor clean | ⏳ TODO |
| 12 | 056 | [sponsor-contract-generate-edge-fn](056-sponsor-contract-generate-edge-fn.md) | 0.5d | gemini, supabase, mdeai-project-gates | curl → PDF bytes returned | ⏳ TODO |
| 13 | 057 | [sponsor-contract-sign-page](057-sponsor-contract-sign-page.md) | 0.5d | frontend-design, supabase, mdeai-project-gates | Browser: Daniela clicks "I agree" | ⏳ TODO |
| 14 | 058 | [sponsor-dispute-ui](058-sponsor-dispute-ui.md) | 0.5d | frontend-design, supabase, mdeai-project-gates | Browser: admin resolves dispute | ⏳ TODO |

---

## ⏳ TODO — PHASE-2-MARKETING (8 tasks, ~6.5 dev-days)

Closed-loop campaign system. **Human-approval gate mandatory before any send.** Topological: 059 → 061 → 060 → 062 → 063/065 → 064 → 066.

| Order | # | Task | Effort | Skills | Verify how | Definition of done |
|---|---|---|---|---|---|---|
| 1 | 059 | [marketing-schema-migration](./events/prompts/059-marketing-schema-migration.md) | 1d | supabase, supabase-postgres-best-practices | `\dt marketing.*`; advisor clean | 13 tables; RLS on all; embedding index on influencers |
| 2 | 061 | [campaign-generate-plan-edge-fn](./events/prompts/061-campaign-generate-plan-edge-fn.md) | 1d | gemini, supabase, mdeai-project-gates | curl → 14 posts in `marketing.posts` | Gemini Flash returns valid JSON; G1 schema enforced |
| 3 | 060 | [campaign-builder-ui](./events/prompts/060-campaign-builder-ui.md) | 1d | frontend-design, supabase, mdeai-project-gates | Browser: Sofía creates + reviews campaign | Post timeline renders; inline edit persists |
| 4 | 062 | [campaign-approve-flow](./events/prompts/062-campaign-approve-flow.md) | 0.5d | supabase, frontend-design, mdeai-project-gates | Browser: admin approves; status banner updates | `campaign_approvals.status='approved'`; Realtime banner |
| 5 | 063 | [postiz-schedule-posts-edge-fn](./events/prompts/063-postiz-schedule-posts-edge-fn.md) | 1d | supabase, mdeai-project-gates | curl → Postiz API receives posts | Unapproved campaign → 400; approved → posts scheduled |
| 5 | 065 | [referral-tracking](./events/prompts/065-referral-tracking.md) | 0.5d | supabase, mdeai-project-gates | curl `?ref=TOKEN` → redirect + click row | IP hash stored; duplicate within 5 min not counted |
| 6 | 064 | [openclaw-outreach-edge-fns](./events/prompts/064-openclaw-outreach-edge-fns.md) | 1d | supabase, mdeai-project-gates | dry_run → payload logged; HMAC verified by VPS | Unapproved → 400; daily cap enforced; PII hashed |
| 7 | 066 | [campaign-ingest-metrics](./events/prompts/066-campaign-ingest-metrics.md) | 1d | supabase, frontend-design, mdeai-project-gates | Analytics dashboard renders all 5 sections | pg_cron runs; CPL calculated; referral leaderboard ranked |

---

## ⏳ TODO — PHASE-2-OPENCLAW (4 tasks, ~3.5 dev-days)

OpenClaw VPS automation layer. Depends on 059 (marketing schema). Build order: 067 → 068 → 069/070 (parallel).

| Order | # | Task | Effort | Skills | Verify how | Definition of done |
|---|---|---|---|---|---|---|
| 1 | 067 | [openclaw-delivery-webhook](./events/prompts/067-openclaw-delivery-webhook.md) | 0.5d | supabase, mdeai-project-gates | curl with valid/invalid HMAC | Invalid sig → 401; delivery_logs upserted; opt-out flips influencer |
| 2 | 068 | [openclaw-whatsapp-concierge](./events/prompts/068-openclaw-whatsapp-concierge.md) | 1d | gemini, supabase, mdeai-project-gates | Simulate inbound "¿Fue contado mi voto?" | Gemini returns Spanish reply in <3s; logs to ai_runs |
| 3 | 069 | [openclaw-influencer-outreach-browser](./events/prompts/069-openclaw-influencer-outreach-browser.md) | 1d | supabase, mdeai-project-gates | dry_run with replied influencer → payload | Only replied contacts targeted; daily cap enforced |
| 3 | 070 | [openclaw-no-show-recovery](./events/prompts/070-openclaw-no-show-recovery.md) | 1d | gemini, supabase, mdeai-project-gates | pg_cron fires for T-12h event; dry_run output | Reminder sent; reply "SÍ" → `attendance_intent='confirmed'` |

---

## ⏳ TODO — PHASE-3-RESTAURANT (2 tasks, ~2 dev-days)

Native reservations replacing external redirect. Build order: 071 → 072.

| Order | # | Task | Effort | Skills | Verify how | Definition of done |
|---|---|---|---|---|---|---|
| 1 | 071 | [restaurant-reservations-schema](./events/prompts/071-restaurant-reservations-schema.md) | 1d | supabase, supabase-postgres-best-practices | `\dt restaurant.*`; advisor clean | 5 tables; UNIQUE on (table_id, date, time); RLS |
| 2 | 072 | [restaurant-booking-edge-fn](./events/prompts/072-restaurant-booking-edge-fn.md) | 1d | supabase, frontend-design, mdeai-project-gates | Race test: 2 concurrent → exactly 1 success | Booking created; confirmation email sent; 409 on double-book |

---

### Cross-cutting expectations on EVERY task

1. **Skills first.** Invoke skills listed in the task's frontmatter `skill:` array BEFORE writing code. Failing to do this has shipped bugs (D3 Rules-of-Hooks; D4 missing FK indexes; D5 GoTrue auth-row schema).
2. **Testing strategy minimums:**
    - Unit: Vitest for any React component with logic, Deno test for edge fn helpers.
    - Integration: SQL/RPC smoke test directly against the live DB via MCP `execute_sql` (proves contract).
    - Boundary: curl matrix on the deployed edge fn (METHOD, malformed body, auth, validation, happy path).
    - **Browser proof: Claude Preview MCP** (`claude-preview-browser-testing` skill) for any user-facing surface — open the page in real Chrome, drive the persona scenario, capture screenshot.
3. **Success criteria explicit.** Acceptance criteria listed in the task spec MUST be checked off before merge. The "Phase 1 acceptance gate" above is the launch checklist.
4. **mdeai-project-gates** runs at end: lint baseline preserved, `npm run build` green, `npm run test` green, bundle delta within budget, edge advisor count steady.

---

## DONE 2026-05-03 — Phase 1 events backend half-shipped (6/10 P0 tasks)

Three tasks shipped earlier (001 schema, 002 wizard, 045 Gemini SDK migration, 046 Gemini skill housekeeping) plus four shipped this session:

- **034 event-staff-link-generator** — minted door-staff JWTs (HS256, dedicated `STAFF_LINK_SECRET`); 3/3 boundary tests; `bump_staff_link_version` RPC migration applied; advisor count down 2 from RPC lockdown
- **004 ticket-checkout** — Stripe checkout session + atomic capacity guard via `ticket_checkout_create_pending` RPC; **12/12 tests pass** (8 HTTP boundary + RPC happy path + OUT_OF_STOCK + cancel + finalize)
- **005 ticket-payment-webhook** — Stripe signature verification + idempotency cache + handlers for `payment_intent.succeeded` + `charge.refunded`; deployed; RPC chain proven via 004 test #12
- **006 ticket-validate** — staff JWT verify + QR JWT verify + revocation check + atomic `ticket_validate_consume` RPC + `event_check_ins` audit log; consumed/already_used/unknown_token paths all verified via SQL

**Manual env-var step required for full E2E** (cannot do via MCP):
```bash
supabase --project-ref zkwcbyxiwklihegjhuql secrets set \
  STAFF_LINK_SECRET="$(openssl rand -base64 48 | tr -d $'\n')" \
  QR_SIGNING_SECRET="$(openssl rand -base64 48 | tr -d $'\n')" \
