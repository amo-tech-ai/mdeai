...
description: "Use this skill when the user is finishing work in the mdeai.co project and needs to know what to verify before committing, pushing, merging, shipping, or deploying. Trigger on any wrap-up signal: \"I'm done\", \"ready to commit/push/ship\", \"about to merge\", \"deploy-check\", \"pre-commit checklist\", \"what should I run first\", \"anything I should run before…\", \"minimum gates\", \"is this going to block me\", or asking whether a lint/test/bundle delta is acceptable. Also trigger right after the user describes finishing changes to src/, supabase/functions/, supabase/migrations/, RLS policies, edge functions, new components, hooks, or chunks — and wants project-specific verification before shipping. Covers the lint + test + build + bundle + edge + browser-proof + schema-advisor gate sequence with mdeai's baselines (lint 444, tests 86, bundle budgets). Do NOT trigger for explaining concepts (what lint/verify_jwt/build mean), debugging a specific error, rolling back commits, or generic Vite questions unrelated to shipping. Source: skills.sh verification-before-completion + 5 run_loop iterations against mdeai eval set."
metadata:
