...
Deep references: [plugin-sdk/](plugin-sdk/) (manifest, worker API, UI).
...
The server rejects manifests where features lack matching capabilities. Every UI slot type, tool, job, and webhook requires a specific capability. See `plugin-sdk/manifest-reference.md` for the full mapping table.
...
For the full PluginContext API: read `plugin-sdk/worker-api-reference.md`.
...
For all hooks, props, patterns, and styling: read `plugin-sdk/ui-reference.md`.
...
- **`plugin-sdk/manifest-reference.md`** — All 37 capabilities, slot types, validation rules, declaration examples
- **`plugin-sdk/worker-api-reference.md`** — Full PluginContext API, lifecycle hooks, sandbox constraints
- **`plugin-sdk/ui-reference.md`** — All 5 hooks, component props, styling, streaming, navigation patterns
