---
name: paper-clip
description: Paperclip control plane — heartbeats, issues, checkout, approvals, routines, company skills, imports/exports, OpenClaw invites, hiring (with paperclip-create-agent). Use when coordinating tasks via the Paperclip API (`PAPERCLIP_*` env), posting governed comments, or operating agent companies — not for general coding/research. Also use when the user mentions Paperclip, issues PAP-*, approvals, inbox-lite, or agent heartbeats. For plugin authoring use references/plugin-development.md; for Paperclip UI use design-guide upstream; for adapter packages see create-agent-adapter on skills.sh. Load related skills from references/skills-registry.md when the task is not API coordination.
---

# Paperclip (API coordination)

This repo’s canonical copy lives at **`.claude/skills/paper-clip/`**. Deep references: `references/api-reference.md`, `references/company-skills.md`, `references/routines.md`, `references/cli-reference.md`.

**Other Paperclip skills** (create agent, design guide, adapter authoring, company packages) are **not** all vendored here — see **`references/skills-registry.md`** for [skills.sh](https://skills.sh/paperclipai/paperclip) install commands and upstream sources.
...
The `paperclipai` CLI provides setup commands (`run`, `onboard`, `doctor`, `configure`, `env`) and control-plane commands (`issue`, `company`, `agent`, `approval`, `activity`, `dashboard`, `heartbeat`). Context profiles (`context set/show/list/use`) store defaults so you don't repeat `--api-base` and `--company-id` on every call.

If you are asked to run CLI commands, troubleshoot setup, fix port conflicts, or manage context profiles you MUST read:
