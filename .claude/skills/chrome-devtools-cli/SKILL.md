...
description: "Drive Chrome via the `chrome-devtools` shell binary (same tools as chrome-devtools-mcp, but invoked from bash) — for one-shot terminal automation, hook scripts, CI smoke checks, or pipelines that pipe snapshot/screenshot/trace output into other tools. Use when the work belongs in a shell script or non-interactive pipeline: capture a Lighthouse audit and save the report, dump network requests to JSON for diff, run a headless smoke flow inside a hook. Do NOT use for: interactive in-session browser work (use chrome-devtools), real signed-in browser flows (use Claude in Chrome), quick preview verification (use Claude Preview MCP), or installing/configuring the MCP server itself (see references/installation.md once)."
---
