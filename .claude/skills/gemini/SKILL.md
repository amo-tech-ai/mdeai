---
name: gemini
description: Use when integrating Gemini AI models in Supabase Edge Functions or Deno. Covers Gemini 3/3.1 models (gemini-3.1-pro-preview, gemini-3-flash-preview, gemini-3.1-flash-image-preview), thinking levels, URL Context, Google Search grounding, structured output, thought signatures, function calling, image generation, document processing, deep search, error handling, and REST API patterns. Use for AI enrichment, data extraction, analysis tasks, and image generation.
---

# Gemini AI Integration

Build AI features using Google's Gemini 3/3.1 models (gemini-3.1-pro-preview, gemini-3-flash-preview, gemini-3.1-flash-image-preview) with URL Context, Google Search grounding, structured outputs, thinking levels, function calling, and image generation.
...
| Model | Use Case | Context (in / out) | Pricing (per 1M tokens unless noted) |
|-------|----------|---------------------|---------------------------------------|
| `gemini-3.1-pro-preview` | Reasoning, deep analysis | 1,048,576 / 65,536 | $2 / $12 (≤200k); $4 / $18 (>200k) |
| `gemini-3-flash-preview` | Fast extract / chat / agents (DEFAULT) | 1,048,576 / 65,536 | $0.50 / $3 |
| `gemini-3.1-flash-lite-preview` | Cheapest classification (e.g. intent router) | 1,048,576 / 65,536 | $0.25 / $1.50 |
| `gemini-3.1-flash-image-preview` | Image gen + edit, text-primary | 128k / 32k | **$0.25 text** / $0.067 per image |
| `gemini-3-pro-image-preview` | Premium image (4K, sharp text rendering) | 65k / 32k | **$2 text** / $0.134 per image |

All Gemini 3 models have a knowledge cutoff of **January 2025** (verbatim per `https://ai.google.dev/gemini-api/docs/gemini-3`).

> **Pricing correction (2026-05-03):** earlier skill versions misattributed Pro-image-preview's pricing/context to Flash-image-preview. Verified against the model cards at `/models/gemini-3.1-pro-preview` and `/models/gemini-3-flash-preview`. The two image models are different products at different price points.

**Capabilities note:** Pro does NOT support image generation. Flash supports Computer Use + Maps grounding (both new in this skill update).
...
Control reasoning depth with `thinkingLevel` (Gemini 3). **Support matrix updated 2026-05-03 from `https://ai.google.dev/gemini-api/docs/thinking`:**

| Level | `gemini-3.1-pro-preview` | `gemini-3.1-flash-lite-preview` | `gemini-3-flash-preview` |
|---|---|---|---|
| `minimal` | ❌ NOT supported | ✅ (default) | ✅ |
| `low` | ✅ | ✅ | ✅ |
| `medium` | ✅ | ✅ | ✅ |
| `high` | ✅ (default — dynamic) | ✅ (dynamic) | ✅ (default — dynamic) |

**Defaults:** Pro + Flash → `high`; Flash-Lite → `minimal`.
...
**Best Practice:** `high` for scoring, math, deep analysis. `low` for chat + simple extraction. `minimal` for sub-second classification (e.g. intent routing).

**`thinkingBudget` (legacy):** verbatim from docs — *"thinkingBudget is accepted for backwards compatibility, using it with Gemini 3 Pro may result in unexpected performance."* Prefer `thinkingLevel` for all new code.
...
**NOTE (corrected 2026-05-03):** Verbatim from `https://ai.google.dev/gemini-api/docs/function-calling`: *"You can enable multiple tools, combining built-in tools with function calling in the same request. Gemini 3 models can combine built-in tools with function calling out-of-the-box, thanks to the tool context circulation feature."*

**Requirements when combining:**
- Set `include_server_side_tool_invocations: true`
- Preserve `thoughtSignature` + `id` + `tool_type` on every part across all turns
- Use `npm:@google/genai@^1.0.0` SDK (it manages signatures automatically)

**One exception (still unsupported):** URL Context + function calling specifically — verbatim from `https://ai.google.dev/gemini-api/docs/url-context`: *"Function calling with URL Context is currently unsupported."*

**Mode `VALIDATED`:** verbatim from function-calling docs — *"Default mode for tool combination. Ensures function schema adherence."* Use it when combining built-in tools with custom functions.

See [`references/tool-combination.md`](references/tool-combination.md) for the full compatibility matrix + multi-tool examples.
...
- [references/tool-combination.md](references/tool-combination.md) - Combining built-in tools + custom function calling (Gemini 3)
- [references/maps-grounding.md](references/maps-grounding.md) - Google Maps grounding, latLng retrievalConfig, pricing
- [references/code-execution.md](references/code-execution.md) - Python sandbox, 30s runtime, pre-approved libs
...
- [ ] **G7:** When combining built-in tools with custom function calling, set `include_server_side_tool_invocations: true` AND preserve `thoughtSignature` + `id` + `tool_type` on every part across all turns (Gemini 3 only — see `references/tool-combination.md`)
- [ ] Maps grounding stays OFF by default; opt in only when geographic intent is detected (free tier 500/day, then $25/1k prompts)
- [ ] Code Execution: never expect network access from inside the sandbox; pre-fetch via Google Search/URL Context first
