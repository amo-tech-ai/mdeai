# Strategic roadmap planning (full workflow)

Five phases: gather inputs → define epics → prioritize → sequence → communicate. Use for quarterly/annual planning and exec-ready roadmaps—not for day-to-day sprint backlog.
...
When running this workflow as a guided conversation, if your workspace has a `workshop-facilitation` skill, use it as the interaction protocol (one-question turns, numbered options, pause/resume).
...
Use a shared template (Notion/Confluence/Doc) for the full fill-in structure if you maintain one.
...
Maintain worked examples in your team wiki or PM tool; keep one **Now / Next / Later** sample and one **quarterly theme** sample for onboarding.
