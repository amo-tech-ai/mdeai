---
name: open-claw
description: OpenClaw self-hosted gateway — install, configure, operate, and debug. Covers WhatsApp/Telegram/Discord/Slack/iMessage/Signal and other channels, Gateway daemon, `openclaw` CLI, models/providers, multi-agent routing, nodes, security, skills, plugins, exec/browser tools, Lobster, memory, heartbeat, and official docs at docs.openclaw.ai. Use for first-time setup (load setup-assistant index), agent workspace design (agent-builder), cost/context tuning (agent-optimize), or doc-accurate answers (expert workflow). Use whenever the user mentions OpenClaw, Gateway, pairing, channels, or `openclaw` errors — even if they do not name this skill.
---

# OpenClaw (hub)

This file is the **maintenance + reference map** for day-to-day operation. For other concerns, load the matching reference **only when needed** (progressive disclosure):

| Concern | Read |
|---------|------|
| **Guided install / DingTalk / Feishu / skills phases** | [references/setup-assistant-index.md](references/setup-assistant-index.md) |
| **Answers grounded in docs.openclaw.ai + GitHub** | [references/expert-docs-workflow.md](references/expert-docs-workflow.md) |
| **New agent workspace (AGENTS.md, SOUL, identity)** | [references/agent-builder-overview.md](references/agent-builder-overview.md) |
| **Optimize cost, context, heartbeat, delegation** | [references/agent-optimize-overview.md](references/agent-optimize-overview.md) |
